...
class Vexation extends BaseStage
{
    ...
    public var allowedCutinAnimations:Array<String> = ['idle', 'singLEFT', 'singDOWN', 'singUP', 'singRIGHTMiss', 'singLEFTMiss', 'singDOWNMiss', 'singUPMiss', 'singRIGHTMiss'];
	...
	override function createPost()
	{
        ...
        cooldownBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'interfaces/game/dodgeBar', () -> return dodgeCooldown.progress, 0, 1);
	...
	override function update(elapsed:Float)
	{
        ...
        if (((FlxG.keys.justPressed.SPACE || MusicBeatState.instance.mobileControls != null && MusicBeatState.instance.mobileControls.current.buttonExtra.justPressed) && !underCooldown) || (game.cpuControlled && canDodge))
        {
            underCooldown = true;
                ...
                switch (value1)
                {
                    case 'Show Opp Strums':
                        if (ClientPrefs.data.opponentStrums)
                            game.opponentStrums.forEachAlive((sturm) -> FlxTween.tween(sturm, {alpha: ClientPrefs.data.middleScroll ? 0.35 : 1.0}, 0.3, {ease: FlxEase.circOut}));
                    case 'Hide First':
                        FlxTween.tween(first, {alpha: 0}, 0.4, {ease: FlxEase.sineOut, onComplete: (_) -> {
    ...
    {
        if (fire != null) fire.pause();
    }

    private function resumeFire()
    {
        if (fire != null) fire.resume();
    }
    ...
    private function set_curStatus(Value:Int)
    {
        ...
        if (game.healthGain < 0) game.healthGain = 0;
