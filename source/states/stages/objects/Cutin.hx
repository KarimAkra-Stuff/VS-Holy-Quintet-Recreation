...
class Cutin extends FlxGroup
{
    ...
    public function new(y:Int, bgName:String, characterName:String, isPlayer:Bool = false, cameraPos:Int)
    {
        ...
        {
            if (isPlayer) PlayState.instance.players.push(character);
            else PlayState.instance.opponents.push(character);
        }
        add(character);
    }
    ...
    override function destroy()
    {
        ...
        {
            if (isPlayer && PlayState.instance.players != null) PlayState.instance.players.remove(character);

            if (!isPlayer && PlayState.instance.opponents != null) PlayState.instance.opponents.remove(character);
        }
